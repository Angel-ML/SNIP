...
QUANT = True
if QUANT:
    from Auto48X.pytorch_quantization.nn import QuantLinear, TensorQuantizer
...
class LinearActivation(nn.Module):
    ...
    def __init__(self, in_features, out_features, act='gelu', bias=True, do_quant=False):
        ...
        inpit :
            ...
            bias: is use bias
            do_quant: is do quant
        """
        super(LinearActivation, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.act_fn = nn.Identity()  # activation fn
        self.biased_act_fn = None  # biased activation fn
        self.bias = None  # bias
        if isinstance(act, str) or (sys.version_info[0] == 2 and isinstance(act, unicode)):  # For TorchScript
            if bias and not 'bias' in act:  # compatibilitya-aaa
                act = 'bias_' + act  #
        ...
        if QUANT and do_quant:
            # replace _input_quantizer and _weight_quantizer
    ...
    def reset_parameters(self):
        # reset LinearActivation parameters
    ...
    def forward(self, input):
        ...
        if self.bias is not None:
            if QUANT and self.do_quant:
                # print(f'Input {input.dtype}, weight {weight.dtype}')
                # a = F.linear(input, weight, None)
                # quant bias
...
def load_tf_weights_in_bert(model, config, tf_checkpoint_path):
    ...
    input:
        model: input model
        config: model config
        tf_checkpoint_path: tensor flow ckpt path
    """
    try:
        import re
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if output_distill is True:
            distill_outputs = [query_layer, key_layer, value_layer]
        else:
            distill_outputs = None
        ...
        return outputs, distill_outputs, attention_scores
...
class BertSelfOutput(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_tensor: torch.Tensor) -> torch.Tensor:
        ...
        if QUANT:
            hidden_states = self.add_local_input_quantizer(hidden_states)
            add_residual = self.add_residual_input_quantizer(input_tensor)
        else:
            add_residual = input_tensor
        hidden_states = self.layernorm(hidden_states + add_residual)
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs, distill_outputs, attention_probs = self.self(
            hidden_states,
            attention_mask,
            head_mask,
        ...
        return outputs, distill_outputs, attention_probs
...
class BertIntermediate(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.dense = LinearActivation(config.hidden_size, config.intermediate_size, act='noact', do_quant=True)
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs, distill_outputs, attention_probs = self.attention(
            hidden_states,
            attention_mask,
            head_mask,
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        total_layernum = self.config.num_hidden_layers
        if teacher_layer is None:
            layer_key = None
        elif teacher_layer < 0:
            layer_key = total_layernum + teacher_layer
        else:
            layer_key = teacher_layer
        for i, layer_module in enumerate(self.layer):
            if layer_key is None:
                output_distill = True
            else:
                if (i == layer_key) or (layer_key == -1):
                    output_distill = True
                else:
                    output_distill = False
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if (add_noise is True) and (i == 1):
                    hidden_states += (torch.randn(hidden_states.size()).to(hidden_states.device) - 0.5) * 0.1
                layer_outputs, distill_outputs, attention_probs = layer_module(
                    hidden_states,
                    attention_mask,
                    layer_head_mask,
                    encoder_hidden_states,
            ...
            all_distill_outputs.append(distill_outputs)
        ...
        # add return all_distill_outputs all_attention_probs for kd
        ...
        ), all_distill_outputs, all_attention_probs
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoder_outputs, all_distill_outputs, all_attention_probs = self.encoder(
            embedding_output,
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        Returns:
            output_dict = SequenceClassifierOutput(
                        loss=loss,
                        logits=logits,
                        hidden_states=outputs.hidden_states,
                        attentions=outputs.attentions,
        ...
        # auto48x bert
        # add all_distill_outputs and all_attention_probs output
        # The rest is the same as transformers
        outputs, all_distill_outputs, all_attention_probs = self.bert(
            input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
        ...
        # output dict add qkv for kd
        output_dict['qkv'] = all_distill_outputs
        # add attention_score for kd
        output_dict['attention_score'] = all_attention_probs
        # add pooled_output for kd
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        end_positions (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            ...
            are not taken into account for computing the loss.
        Return:
            output_dict = QuestionAnsweringModelOutput(
            loss=total_loss,
        ...
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        outputs, qa_all_distill_outputs, qa_all_attention_probs = self.bert(
            input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
        ...
        logits = self.qa_outputs(sequence_output).float()
        ...
        output_dict['qkv'] = qa_all_distill_outputs
        output_dict['attention_score'] = qa_all_attention_probs
        return output_dict
