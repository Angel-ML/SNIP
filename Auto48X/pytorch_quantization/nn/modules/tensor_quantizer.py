...
from Auto48X.pytorch_quantization.tensor_quant import QuantDescriptor, tensor_quant, fake_tensor_quant
from Auto48X.pytorch_quantization.nn.modules.clip import Clip

from Auto48X.pytorch_quantization import calib

import Auto48X.pytorch_quantization.utils as quant_utils
...
class TensorQuantizer(nn.Module):
    ...
    def load_calib_amax(self, *args, **kwargs):
        ...
        *args and **kwargs are directly passed to compute_amax, except "strict" in kwargs. Refer to
        ...
        calib_amax = self._calibrator.compute_amax(*args, **kwargs).float()
        # print(f'loading calib_amax gets {calib_amax}')
        if calib_amax is None:
            ...
            if not strict:
                ...
                print(f'loading calib_amax gets {self}')
        ...
        #logging.warning("Call .cuda() if running on GPU after loading calibrated amax.")

        if not hasattr(self, '_amax'):
            # print(calib_amax)
            # print(self._amax, calib_amax)
            self.register_buffer('_amax', calib_amax.data)
        else:
            # print(self._amax,calib_amax)
            self._amax.copy_(calib_amax)

    def switch_learn_amax(self):
        self._amax = self._amax.clone().detach().requires_grad_(True)


    # def switch_optimal_amax(self):
    def convert_amax_f32(self):
        if self._amax is not None:
            self._amax = self._amax.float()
            #print(self._amax.dtype)
    ...
    def _fb_fake_quant(self, inputs, amax):
        ...
        #logging.warning("Use Pytorch's native experimental fake quantization.")
    ...
    def _quant_forward(self, inputs):
        ...
        if self._fake_quant:
            if not TensorQuantizer.use_fb_fake_quant:
                ...
                # outputs = fake_tensor_quant(inputs, self._amax, self._num_bits, self._unsigned, self._narrow_range)
                # print('Inside the output is:', outputs)
            else:
                if inputs.dtype == torch.half or amax.dtype == torch.half:
                    raise Exception("Exporting to ONNX in fp16 is not supported."
                                    " Please export in fp32, i.e. disable AMP.")
