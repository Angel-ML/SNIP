...
class ClipFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if clip_value_min.requires_grad or clip_value_max.requires_grad:
            pass
            #logging.warning("Learning clip min/max is experimental, use at your own risk :).")
        ...
        grad_clip_value_min = (grad_output * (1. - min_mask)).sum().to(clip_value_min.dtype) \
            if clip_value_min.requires_grad else None
        grad_clip_value_max = (grad_output * (1. - max_mask)).sum().to(clip_value_min.dtype) \
            if clip_value_max.requires_grad else None
