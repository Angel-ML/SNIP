...
import logging
import torch
from Auto48X.pytorch_quantization.calib.calibrator import _Calibrator
from Auto48X.pytorch_quantization import utils as quant_utils
...
class MaxCalibrator(_Calibrator):
    ...
    def collect(self, x):
        ...
        """
        if torch.min(x) < 0.:
            logging.warning("Calibrator encountered negative values. It shouldn't happen after ReLU. Make sure "
                            "this is the right tensor to calibrate.")
            x = x.abs()
        ...
        # print(f'local_max is {local_amax}')
        if local_amax is None:
            print('Find None and exit')
            exit()
        if self._calib_amax is None:
            self._calib_amax = local_amax.clone()
        else:
    ...
    def compute_amax(self):
        ...
        # print(f'local max is {self._calib_amax}')
