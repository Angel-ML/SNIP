...
class FakeTensorQuantFunction(Function):
    ...
    def forward(ctx, inputs, amax, num_bits=8, unsigned=False, narrow_range=True):
        ...
        outputs, scale = _tensor_quant(inputs, amax, num_bits, unsigned, narrow_range)
        ctx.save_for_backward(inputs, amax, outputs, scale)
        if amax.numel() > 1 and len(scale.shape) == 1:
            return outputs / scale[:, None].to(inputs.dtype)
        else:
            return outputs / scale.to(inputs.dtype)

    @staticmethod
    def backward(ctx, grad_outputs):
        inputs, amax, outputs, scale = ctx.saved_tensors
        # print(outputs)
        zero = grad_outputs.new_zeros(1)
        # print(f'weight size is {inputs.shape}, scale size is {scale.shape}')
        if amax.numel() > 1 and len(scale.shape) == 1:
            grad_inputs = torch.where(inputs.abs() <= amax[:, None], grad_outputs, zero)
        else:
            grad_inputs = torch.where(inputs.abs() <= amax, grad_outputs, zero)
        if amax is not None:
            if  amax.numel() == 1:
                amax_grad = torch.sum((outputs.float() - inputs.float() * scale.float()) * outputs.float() /
                                      scale.float() / outputs.numel() * amax.float()).float()
            else:
                # amax_grad = (torch.sum((outputs.float() - inputs.float() * scale[:,None].float()) * outputs.float()
                # / scale[:,None].float() * amax.float(), dim=1,keepdim=True) / outputs.size()[1]).half()
                amax_grad = None
            # print(amax_grad)
        else:
            amax_grad = None
        # print(grad_inputs.size())

        return grad_inputs, amax_grad, None, None, None



class FakeTensorQuantFunction2(Function):
    """Fake version of TensorQuantFunction
    See comments of TensorQuantFunction, arguments are the same.
    """
    ...
    def backward(ctx, grad_outputs):
        ...
        if amax is not None:
            if  amax.numel() == 1:
                amax_grad = torch.sum((outputs.float() - inputs.float() * scale.float()) / outputs.numel()).\
                    type(inputs.dtype)
            else:
                amax_grad = (torch.sum((outputs.float() - inputs.float() * scale.float()) *
                                       outputs.float() / scale.float() * amax.float(), dim=1, keepdim=True) /
                             outputs.size()[1]).type(inputs.dtype)
            # print(amax_grad)
        else:
            amax_grad = None
        # print(grad_inputs.size())
...
def _tensor_quant(inputs, amax, num_bits=8, unsigned=False, narrow_range=True):
    ...
    #
    # print(f'Min is {min_bound}, max is {max_bound}')
    # exit()
    ...
    if amax.numel() > 1 and len(scale.shape) == 1:
        outputs = torch.clamp((inputs * scale[:, None]).round_(), min_bound, max_bound)
    else:
    ...
    if input_dtype == torch.half:
        ...
        outputs = outputs.type(torch.bfloat16)
