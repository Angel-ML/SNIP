    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            if args.model_type in ['xlnet', 'xlm']:
                ...
                # print(model)
                # print(type(outputs))
                # print(type(outputs["start_logits"]))
                # print(outputs["start_logits"])
                # print(type(outputs["end_logits"]))
...
def get_args():
    ...
    parser.add_argument("--n_best_size", default=20, type=int,
                        help="The total number of n-best predictions to generate in "
...
def do_train(args, train_dataset, t_total, model, train_dataloader, engine, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            num_input = torch.sum(inputs["input_ids"], dim=1)
            if not args.pure_finetune:
                loss = engine.add_knowledge_distillation(loss, num_input=num_input, T=1)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    results = evaluate(args, model, engine, tokenizer)

                    # Save model checkpoint
                    output_dir = os.path.join(args.output_dir, 'em-{}_f1-{}_checkpoint-{}'.format(
                        results['exact'], results['f1'], global_step))
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    if max_acc < results['exact']:
                        max_acc = results['exact']
                        # Take care of distributed/parallel training
                        model_to_save = model.module if hasattr(model,
                                                                'module') else model
                        model_to_save.save_pretrained(output_dir)
                        torch.save(args, os.path.join(output_dir, 'training_args.txt'))
...
def main():
    ...
    config = atbert.BertConfig.from_pretrained("bert-base-uncased")
    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased", do_lower_case=args.do_lower_case)
    model = atbert.BertForQuestionAnswering(config=config)
    teacher_model = atbert.BertForQuestionAnswering(config=config)
    ...
    model, optimizer, teacher_model, engine = Auto48X.Auto48Init(args, model, optimizer=optimizer,
                                                                 ...
                                                                 teacher_model=teacher_model, lr_scheduler=scheduler)
    ...
    if args.do_train:
        ...
        do_train(args, train_dataset, t_total, model, train_dataloader, engine, tokenizer)
